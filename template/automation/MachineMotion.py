...
def HTTPSend(host, path, data=None) :
    ...
    while True :
        ...
        try :
            ...
            # TODO: Put this back
            logger.exception("Could not get {}: {}".format(path, traceback.format_exc()))
...
def HTTPSendStreamed(host, path, data=None, onDataReceived=None, onConnectReceived=None) :
    logger = logging.getLogger(__name__)
    # Note:
    #   Streams messages until we no longer receive content.
    #   The intent of retrying upon failure here is primarily to reconnect to a dead or unreachable server.
    #   The assumption is that an exception at this level reflects a server failure not to be expected by the client.
    #   This behavior could be made optional.
    while True :
        ...
        try :
            ...
            if None == data:
                ...
                lConn.request("POST", path, data, {"Content-type": "application/json"})
                
            if onConnectReceived:
                onConnectReceived(lConn)
            
            lResponse = lConn.getresponse()
            idx = 0
            while True:
                try:
                    num_bytes_read = lResponse.fp.readline(65536 + 1)
                    if lResponse is None or lResponse.fp is None:
                        break
                    
                    content = lResponse.fp.readline(65536 + 1).decode('utf-8').strip()
                    if len(content) == 0:
                        break
                    
                    if content == "BEGIN":
                        logger.debug("Received the begin notification for this batch.")
                        # This is just a sanity thing to make sure we're receiving a clean slate
                        continue

                    if not onDataReceived is None:
            ...
            lConn.close()
            return "echo: ok" # TODO: This is faked for now, but that should be ok
        except Exception :
            # TODO: Put this back
            logger.exception("Could not get {}: {}".format(path, traceback.format_exc()))
            #logging.warning("Could not GET %s: %s", path, traceback.format_exc())
...
class GCode:
    ...
    def __send_streamed__(self, cmd, data=None, on_data_received=None, onConnectReceived=None) :

        return HTTPSendStreamed(self.myIp + self.libPort, cmd, data, on_data_received, onConnectReceived)
    ...
    #
    # Function to send a raw G-Code ASCII command list
    # @param gCodeList --- Description: gCodeList is a list of strings representing the G-Code command to send to the controller. Type: string[].
    # @param onGcodeProcessed --- Description: function(string, int) containing the response and the line number
    # @status
    #
    def __emit_gcode_batch__(self, gCodeList, onGcodeProcessed, onConnectReceived) :

        rep = self.__send_streamed__("/gcodeBatch", json.dumps(gCodeList), onGcodeProcessed, onConnectReceived)
...
class MachineMotion :
    ...
    def __getStepValue(self, step_str, param):
        try:
            retval_str = ""
            param_idx = step_str.index(param)
            param_idx = param_idx + 1 # Move past the param delimeter
            param_idx = param_idx + 1 # Move past the colon
            while step_str[param_idx].isdigit() or step_str[param_idx] is ' ' or step_str[param_idx] is '-' or step_str[param_idx] is '.':
                if step_str[param_idx] is not ' ':
                    retval_str += step_str[param_idx]
                param_idx = param_idx + 1
            return float(retval_str)
        except:
            return None
    ...
    def getCurrentSteps(self):
        ...
        if ( "echo" in reply and "ok" in reply ) :
            try:
                count_start = reply.index("Count")
                step_value = reply[count_start:].replace(" ", "") # The seconds instance of X should define the steps values
                steps[1] = self.__getStepValue(step_value, "X")
                steps[2] = self.__getStepValue(step_value, "Y")
                steps[3] = self.__getStepValue(step_value, "Z")
            except: 
                raise Exception('Could not find steps')
    ...
    def emitgCodeBatch(self, gCodeList, onLineProcessed, onConnectReceived):
        '''
        desc: Executes a batch of raw gCode on the controller.
        params:
            gCodeList:
                desc: The g-code lines that will be passed directly to the controller.
                type: string[]
            onLineProcessed:
                desc: callback whenever a new command is executed
        ...
        reply = self.myGCode.__emit_gcode_batch__(gCodeList, onLineProcessed, onConnectReceived)

        if ( "echo" in reply and "ok" in reply ) : pass
        else : raise Exception('Error in gCode batch execution (reply: %s)' % reply)
    ...
    def waitForMotionCompletion(self):
        ...
        try:
            ...
            self.logger.error("Error emitting gcode dude.")
            reply = ["echo", "ok", "COMPLETED"]

        #Check if not error message
        self.logger.info("Wait for motion completion reply: %s", reply)
    ...
    def configMachineMotionIp(self, mode = None, machineIp = None, machineNetmask = None, machineGateway = None):
        ...
        if(mode == NETWORK_MODE.static):
            if (machineIp is None) or (machineNetmask is None) or (machineGateway is None) :
               self.logger.warning("NETWORK ERROR: machineIp, machineNetmask and machineGateway cannot be left blank in static mode")
    ...
    def digitalRead(self, deviceNetworkId, pin) :
        ...
        if ( not self.isIoExpanderInputIdValid( deviceNetworkId, pin ) ):
            self.logger.warning("DEBUG: unexpected digital-output parameters: device= " + str(deviceNetworkId) + " pin= " + str(pin))
            return
    ...
    def digitalWrite(self, deviceNetworkId, pin, value) :
        ...
        if ( not self.isIoExpanderOutputIdValid( deviceNetworkId, pin ) ):
            self.logger.warning("DEBUG: unexpected digitalOutput parameters: device= " + str(deviceNetworkId) + " pin= " + str(pin))
            return
    ...
    def readEncoder(self, encoder, readingType=ENCODER_TYPE.real_time) :
        ...
        if (not self.isEncoderIdValid(encoder)):
            self.logger.warning ( "DEBUG: unexpected encoder identifier: encoderId= " + str(encoder) )
            return
    ...
    def setBatchNotificationState(self, toggleOn):
        '''
        desc: Toggles V6 on/off.
        params:
            toggleOn:
                type: boolean
                desc: Sets the status of V6.
        '''
        if (toggleOn):
